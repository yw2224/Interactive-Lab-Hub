...
var apiai = require('apiai')('b85e65a465b045bd9a5cefbabdaeb220');
var serverPort = 8000;
...
io.on('connect', function(socket) {
  ...
  socket.on('loaded', function() { // we wait until the client has loaded and contacted us that it is ready to go.
    ...
    socket.emit('answer', "Hey, hello I am \"*_*\" a simple chat bot example."); //We start with the introduction;
    setTimeout(timedQuestion, 5000, socket, "Press the button to talk to me!"); // Wait a moment and respond with a question.
  });
...
//--------------------------CHAT BOT FUNCTION-------------------------------//
function aiBot(data, socket) {
    let apiaiReq = apiai.textRequest(data, {
      sessionId: "ARANDOMSTRINGFORSESSIONID"
    });

    apiaiReq.on('response', (response) => {
      var aiText = response.result.fulfillment.speech;
      socket.emit('answer', aiText); // Send the result back to the browser!
      console.log(aiText);
      setTimeout(timedQuestion, 5000, socket, aiText);
    });
